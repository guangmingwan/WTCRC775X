...
void SetRFCtrlReg(void)
{
	...
	//sec  64h
	// 7          6        5       4        3             2          1        0
	// [ext agc ] |  reserved   000    |  reserved 0 | [ant SEL]  |  SAD |   IPDq
	...
	{
		rawFMAT = nFMAT;
		bool bFMiPD = IS_PHASE_DIVERSITY_ENABLED(rawFMAT);
		
		if(bFMiPD) {
				DISABLE_PHASE_DIVERSITY(rawFMAT);
		}
		
		data &= 0x7F;//01111111
		data2 = data;
		data |= (rawFMAT << 2) & 0x04;//100
		if(data & 0x04)//100 if pri SEL ant 1
				data2 &= 0xFB;//11111011 sec SEL ant 0
		else
...
void WriteChFreq(bool bAdd)
{
	...
	{  // Add ch
		if (nBand <= BAND_SW)
			u16 = (nBandFreq[nBand] - nBandFMin[nBand]);
		else
			u16 = ((nBandFreq[nBand] - nBandFMin[nBand]) / 5);
	}
	else  // Delete ch
		u16 = 0xffff;
		...
		if ((timer - nAutoSyncTimer) >= TIMER_AUTOSYNC)
		{
			...
			if (nAutoSyncFreqs)
			{
				for (i8 = 0; i8 < NUM_BANDS; i8++)
				{
					...
					{
						if (i8 <= BAND_SW)
							u16 = (nBandFreq[i8] - nBandFMin[i8]);
						else
							u16 = ((nBandFreq[i8] - nBandFMin[i8]) / 5);
